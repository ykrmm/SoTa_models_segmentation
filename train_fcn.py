...
voc = dataset.VOC_Dataset(batch_size=1,dataroot=dataroot,year='2011')
...
for ep in range(n_epochs):
    ...
    for i,(x,mask) in enumerate(dataloader_train):
          ...
          if (i%20==0 and i!=0) or (i==len(dataloader_train)-1):
            optimizer.zero_grad()
    ...
    for i,(x,mask) in enumerate(dataloader_val):
          ...
          #mask = mask.squeeze()
          loss += criterion(pred,mask)
          if i%20==0 and i!=0 or (i==len(dataloader_val)-1):
            iou,treshold = metric.iou(pred.argmax(dim=1),mask)
            p_acc = metric.scores(pred.argmax(dim=1),mask)["Pixel Accuracy"]
...
try:
  plot.plot_figures(loss_train,loss_test,iou_train,iou_test,model_name='FCN 32')
except:
  print("plot bug")
